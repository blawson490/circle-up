...
import EditableDate from "@/app/ui/admin/editableDate";
...
export default async function DeckEditPage({ params }: DeckPageProps) {
  ...
  async function updateTitle(newText: string) {
    "use server";
    try {
      await updateDeckTitle(deckId, newText);
    } catch (error) {
  ...
  async function updateDate(newDate: Date | null) {
    "use server";
    try {
      if (newDate) {
        await updateDeckDate(deckId, newDate.toISOString());
      } else {
        await updateDeckDate(deckId, null);
      }
    } catch (error) {
      console.error("Failed to update deck date:", error);
      // You might want to throw an error here or handle it differently
      throw new Error("Failed to update deck date");
    }
  }
  ...
  async function handleCardsChange(deckId: number, newCards: Card[]) {
    ...
    try {
      ...
      revalidatePath(
        `/admin/edit/collection/${updatedDeck.collectionId}/deck/${deckId}`
      );
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  <AlertDialogTrigger className="w-full">
                    <DropdownMenuItem className="text-red-500 font-semibold focus:text-red-500 focus:font-semibold w-full">
                      Delete
                    </DropdownMenuItem>
                  </AlertDialogTrigger>
                </DropdownMenuContent>
        ...
        <main className="space-y-8 flex-1 flex flex-col overflow-x-hidden overflow-y-auto bg-gray-50 px-4 py-4 pb-24 items-center">
          <div className="flex flex-col justify-center w-full max-w-3xl">
            <div className="flex flex-col justify-start w-full py-6">
              <EditableTitle text={deck.title} updateText={updateTitle} />
              ...
              <EditableDate date={deck.date} updateDate={updateDate} />
            </div>
